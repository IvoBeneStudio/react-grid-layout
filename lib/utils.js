...
export type CompactType = ?("horizontal" | "vertical" | "horizontalList");
...
function resolveCompactionCollision(
  ...
  console.log("-- RESOLVE COMPACTION COLLISION --");
...
export function compactItem(
  ...
  console.log("-- COMPACT ITEM --");

  const compactV = compactType === "vertical";
  const compactH = compactType === "horizontal";
  const compactHList = compactType === "horizontalList";
  if (compactV) {
  ...
  } else if (compactHList) {
    l.y = Math.min(bottom(compareWith), l.y);

    while (l.x < cols - 1 && !getFirstCollision(compareWith, l)) {
      l.x++;
    }
...
export function moveElement(
  ...
  const movingUp =
    compactType === "vertical" && typeof y === "number"
      ...
      ? oldX >= x
      : compactType === "horizontalList" && typeof y === "number"
      ? oldY >= y
      : false;
...
export function moveElementAwayFromCollision(
  ...
  console.log("-- MOVE ELEMENT AWAY FROM COLLISION --");

  const compactH = compactType === "horizontal";
  const compactHList = compactType === "horizontalList";
  // Compact vertically if not set to horizontal
  const compactV =
    compactType !== "horizontal" && compactType !== "horizontalList";
  ...
  if (isUserAction) {
    ...
    if (!getFirstCollision(layout, fakeItem)) {
      ...
      return moveElement(
        layout,
        itemToMove,
        compactH || compactHList ? fakeItem.x : undefined,
  ...
  return moveElement(
    layout,
    itemToMove,
    compactH || compactHList ? itemToMove.x + 1 : undefined,
